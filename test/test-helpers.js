...
function seedThingsTables(db, users, things, reviews = []) {
  //use a transaction to group the queries and auto rollback any failures
  return db.transaction(async trx => {
  ...
  //   .insert(users).then(() => db.into('thingful_things').insert(things))
  //   .then(() => reviews.length && db.into('thingful_reviews')
  //     .insert(reviews));
}
